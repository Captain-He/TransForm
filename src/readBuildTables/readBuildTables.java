...
public class readBuildTables {
	...
	static final int max_X = 1000;//数组中每张表行数的最大值
	static final int max_Y = 100;//数组中每张表列数的最大值
	
	static int shigong = 0;// 查找 施工总图记录表 存于第一维索引存于shigong
	static int wenzhao = 0;// 查找 测温传感器和测温集中器对照表 存于第一维索引存于wenzhao
	static int wenpei = 0;// 查找 测温集中器现场施工配置记录 存于第一维索引存于wenpei
	static int dianli = 0;// 查找 电力仪表 存于第一维索引存于dianli
	static int caiji = 0;// 查找 采集PC现场施工配置记录 存于第一维索引存于wenpei
	static int tongguan = 0;  //查找 通信管理机表 存于第一维索引存于tongguan
...
//		readBuildTables("C:/Users/Admin/Desktop/施工记录v0.5-模拟.xlsx");
//		table6();
...
//	}
	...
	public  String [][][] readTables(String dir) {
		...
		for (int i = 0; i < BuildTable.length; i++) {
			if (BuildTable[i][0][0].equals("施工总图记录表")) {
				shigong = i;
			}
			if (BuildTable[i][0][0].equals("测温传感器和测温集中器对照表")) {
				wenzhao = i;
			}
			if (BuildTable[i][0][0].equals("测温集中器现场施工配置记录")) {
				wenpei = i;
			}
			if (BuildTable[i][0][0].equals("通信管理机现场施工配置记录表")) {
				tongguan= i;
			}
			if (BuildTable[i][0][0].equals("电力仪表现场施工配置记录表")) {
				dianli = i;
			}
			if(BuildTable[i][0][0].equals("采集PC现场施工配置记录")){
				caiji = i;
			}
		}//到此sheet级结束
        table1();
